...
static bool vmmap(task_t task, uintptr_t start, uintptr_t end, uint32_t depth) {
    int pid = -1;
...
//#if defined(__x86_64__)
        errno = 0;
        //获取文件名这个api可以在ios上强行用，但是并不怎么靠谱
        int ret = proc_regionfilename(pid, address, filename, sizeof(filename));
        if (ret <= 0 || errno != 0) {
            filename[0] = 0;
        }
//#endif
